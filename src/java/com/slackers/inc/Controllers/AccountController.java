...
public class AccountController {
    ...
    public boolean editAccount(HttpServletRequest request, HttpServletResponse response) {
        try {
            if (db.writeEntity(this.user, "email")) {
                User usr = this.user;
                if (usr != null) {
                    Cookie prevName = new Cookie(PREV_UNAME, user.getEmail());
                    Cookie uName = new Cookie(EMAIL_COOKIE, user.getEmail());
                    Cookie uPass = new Cookie(PASSWORD_COOKIE, user.getPassword());
                    uName.setMaxAge(60 * 60 * 24);
                    uPass.setMaxAge(60 * 60 * 24);
                    prevName.setMaxAge(60 * 60 * 24);
                    uName.setPath("/");
